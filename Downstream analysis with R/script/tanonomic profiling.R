...
#flipping the heatmap

# Transpose the matrix (t) so species are on the left (Y-axis)
pheatmap(t(otu_heatmap_ordered), 
         cluster_rows = TRUE,    
         cluster_cols = FALSE,   
         scale = "column",       # Scale by column since samples are now columns
         fontsize_row = 7,       # Small font for the many species names
         main = "Dominant Taxa (Bray-Curtis)")
