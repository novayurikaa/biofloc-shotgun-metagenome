...
# --- FINAL CLEANUP FOR INTERVIEW ---
# 1. Identify the most dominant gene functions in Bin 1
top_genes_bin1_final <- egg_annotations_fixed %>%
  filter(Bin == "ECFFIKGN") %>%
  filter(Description != "-") %>%  # Cleans the unannotated genes
  group_by(Description) %>%
  summarise(Gene_Count = n(), .groups = "drop") %>%
  arrange(desc(Gene_Count)) %>%
  slice_max(order_by = Gene_Count, n = 15)

# 2. Visualization for the Interview
ggplot(top_genes_bin1_final, aes(x = reorder(Description, Gene_Count), y = Gene_Count, fill = Gene_Count)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  scale_fill_gradient(low = "#74add1", high = "#023858") +
  theme_minimal() +
  labs(title = "Functional Dominance: Bin 1 (ECFFIKGN)",
       subtitle = "Top 15 identified functions in the primary MAG",
       x = NULL, y = "Number of Genes") +
  theme(axis.text.y = element_text(size = 10))


library(stringr)
library(ggplot2) #we cannot produce the plot

# 1. Prepare the data with text wrapping
library (stringr)
top_genes_clean <- top_genes_bin1_final %>%
  mutate(Description = str_wrap(Description, width = 40)) 
 
# 2. Create the professional plot
ggplot(top_genes_clean, aes(x = reorder(Description, Gene_Count), y = Gene_Count, fill = Gene_Count)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  scale_fill_gradient(low = "#74add1", high = "#023858") +
  theme_minimal() +
  labs(title = "Functional Dominance: Bin 1 (ECFFIKGN)",
       subtitle = "Top 15 identified functions (Text Wrapped for Clarity)",
       x = NULL, 
       y = "Number of Genes") +
  theme(
    axis.text.y = element_text(size = 9, lineheight = 0.8), # Adjust text size and line spacing
...
# 2. Final High-Clarity Plot


# 3. Quick Taxa Check (Mental Link)
# View the best taxonomic hits from the orthologs
head(egg_annotations_fixed %>% filter(Bin == "ECFFIKGN") %>% select(eggNOG_OGs), 10)
